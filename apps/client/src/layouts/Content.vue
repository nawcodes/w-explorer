  ...
  <main class="p-4 sm:ml-64 mt-[3.875rem]">
    ...
    <div class="fixed  top-[3.875rem] right-0 sm:left-64 left-0 bg-gray-100 z-40">
      <div class="px-4 py-3">
        ...
        <div class="flex justify-between items-center mb-4">
          <Breadcrumb :path="currentPath" @navigate="handleBreadcrumbNavigation" />
        </div>

        <div class="flex justify-between items-center gap-2">
          <CreateMenu :is-open="isCreateMenuOpen" @create-folder="handleCreateFolder" @upload-file="handleUploadFile"
    ...
    <div class="pt-[8rem]">
      <div class="bg-white rounded-lg shadow p-6 h-full">
        <!-- Empty State -->
        <EmptyState v-if="isEmpty" title="No items"
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
          <ContentItem v-for="item in sortedItems" :key="item.id" :item="item" @click="handleItemClick" />
        </div>
      </div>
    </div>

    <!-- File Detail Modal -->
    <FileDetailModal v-if="selectedFile" :is-open="isFileDetailModalOpen" :file="selectedFile"
      @close="isFileDetailModalOpen = false" @download="handleFileDownload" />
  </main>
</template>
